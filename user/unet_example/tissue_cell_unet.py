...
from user.unet_example.unet import UNet
...
class PytorchUnetCellModel():
    ...
    def __init__(self, metadata):
        ...
        # self.unet = smp.Unet(encoder_depth=5,
    ...
    def load_checkpoint(self):
        ...
        _path_to_checkpoint = os.path.join(_curr_path, "checkpoints/unet_cell1.pth")
    ...
    def prepare_input(self, cell_patch):
        ...
        transform = transform(image= cell_patch)
        return img_transform(transform['image']).unsqueeze(0).cuda()
    ...
    def find_cells(self, heatmap):
        ...
        # tensor_arr = heatmap[0,:,:,:]
        # tensor_arr = torch.argmax(tensor_arr,dim=0)
        # print(tensor_arr.shape)
        # tensor_arr = tensor_arr.cpu().detach().numpy()
        peaks = feature.peak_local_max(
            arr, min_distance=8, exclude_border=0, threshold_abs=0.0
        ) # List[y, x]
        # print(len(peaks))
    ...
    def __call__(self, cell_patch, tissue_patch, pair_id):
        ...
        # print(logits.shape)
        # l=torch.argmax(logits.squeeze(0),dim=0)
        # print(l[l==0].shape)
